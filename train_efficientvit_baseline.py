...
from models.efficientvit_baseline import EfficientViTSimple
from dataset_cornell_pure import CornellPureDataset
from train_real import combined_loss, train_epoch, evaluate
...
def main():
    ...
    model = EfficientViTSimple().to(device)
    ...
    for epoch in range(150):  # 增加训练轮次至150
        ...
        if metrics['rmse'] < best_rmse:
            ...
            torch.save(checkpoint, 'best_efficientvit_baseline_model.pth')
    ...
    print("\n" + "="*60)
    print("训练完成！最佳性能:")
    checkpoint = torch.load('best_efficientvit_baseline_model.pth', weights_only=False)
